...
export const createOrderSchema = z.object({
  ...
  discount: z.coerce.number().min(0, 'Discount cannot be negative').max(1000000, 'Discount cannot exceed ₱1,000,000').default(0),
...
export const updateOrderSchema = z.object({
  ...
  discount: z.coerce.number().min(0, 'Discount cannot be negative').max(1000000, 'Discount cannot exceed ₱1,000,000').optional(),
  isArchived: z.boolean().optional(),
  deliveryDriverName: safeOptionalString(200),
  deliveryDate: z.string().optional().or(z.literal('')),
});
