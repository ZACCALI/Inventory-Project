...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  const handleSubmit = async () => {
    ...
    if (fulfillmentMode === 'walkin') {
      const selectedCust = customers.find(c => c.id === customerIdToUse);
      if (selectedCust && fallbackCustomerName !== selectedCust.name) {
        customerIdToUse = '';
      }
      if (!fallbackCustomerName && !customerIdToUse) {
        fallbackCustomerName = 'BAIE';
      }
    } else {
    ...
    try {
      ...
      if (isOffline || networkFailed) {
        ...
        const selectedObj = customerIdToUse ? customers.find(c => c.id === customerIdToUse) : null;
        const resolvedName = manualCustomerName.trim()
          || selectedObj?.name
    ...
    <>
      <div className="pos-layout" style={{ maxWidth: '100%', width: '100%', overflowX: 'hidden', minWidth: 0, boxSizing: 'border-box' }}>
        ...
        <div className="walkin-left-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowY: 'auto', overflowX: 'hidden', minWidth: 0, maxWidth: '100%' }}>
          ...
          <div className="page-header mobile-col" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div>
